...
ENTERPRISE_HTML_PATH = BASE_DIR / "supersonic_enterprise_dashboard_v2_3_관리자연동.html"
ADMIN_HTML_PATH = BASE_DIR / "supersonic_enterprise_admin_v1.html"
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/health":
            self._json(HTTPStatus.OK, {"ok": True, "service": "SUPERSONIC Enterprise API", "version": "v2.3"})
            return

        if path in {"/admin", "/admin.html"}:
            admin_user = self._require_super_admin()
            if not admin_user:
                return
    ...
    def do_POST(self) -> None:
        ...
        if path == "/api/admin/user/reset-password":
            ...
            salt_b64, hash_b64 = hash_password(password)
            with db_connect() as conn:
                cur = conn.execute("UPDATE users SET password_salt=?,password_hash=? WHERE id=?",
...
def serve() -> None:
    ...
    print("SUPERSONIC Enterprise API Core v2.3.1")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SUPERSONIC Enterprise API Core v2.3.1")
