...
def team_of(name):
    ...
    sonic = {norm(x) for x in SONIC_TEAM_RIDERS}
    number = {norm(x) for x in NUMBER_TEAM_RIDERS}
    maeum = {norm(x) for x in MAEUM_TEAM_RIDERS}
...
def phone_digits(value):
    """전화번호를 숫자만 남겨 중복 비교용 키로 변환합니다."""
    return re.sub(r"\D", "", str(value or ""))


def final_dedupe_riders(riders):
    """
    달서A에서 확인된 최종 문제와 동일하게 전화번호 표기 차이(하이픈/공백)를 최종 제거합니다.
    추가로 달서B에서 발견된 케이스처럼 같은 이름이 2개인데 한쪽 전화번호만 하이픈 없이 잡힌 경우는
    DOM 복제/비정상 행일 가능성이 높아 하이픈 없는 쪽을 제외합니다.
    단, 같은 이름이라도 둘 다 정상 하이픈 전화번호이면 동명이인 가능성이 있어 유지합니다.
    """
    result = []
    seen_phone = set()

    for r in riders:
        pkey = phone_digits(r.get("phone", ""))
        if not pkey:
            print("전화번호 없음 제외:", r.get("name", ""), r.get("phone", ""))
            continue
        if pkey in seen_phone:
            print("중복 기사 제외:", r.get("name", ""), r.get("phone", ""))
            continue
        seen_phone.add(pkey)
        result.append(r)
    ...
    for name, rows in by_name.items():
        if not name or len(rows) < 2:
            continue
        hyphen_rows = [r for r in rows if "-" in str(r.get("phone", ""))]
        plain_rows = [r for r in rows if "-" not in str(r.get("phone", ""))]
        if hyphen_rows and plain_rows:
            for r in plain_rows:
                remove_ids.add(id(r))
                print("같은이름 비정상 전화번호 제외:", r.get("name", ""), r.get("phone", ""))

    if remove_ids:
        result = [r for r in result if id(r) not in remove_ids]
...
def collect_all_pages_by_dom(page):
    ...
    all_riders = final_dedupe_riders(all_riders)
