...
DATA_FILE = BASE_DIR / "data_haneul.json"
HTML_FILE = BASE_DIR / "haneul.html"
WEEKLY_FILE = BASE_DIR / "weekly_haneul.json"
...
def git_push():
    ...
    subprocess.run(["git", "add", "data_haneul.json", "haneul.html", "c_h.py", "logo.png"], cwd=BASE_DIR)

    if WEEKLY_FILE.exists():
        subprocess.run(["git", "add", "weekly_haneul.json"], cwd=BASE_DIR)
...
def run_update(page):
    ...
    print(f"하늘팀 접속중: {data['teams']['하늘팀']['summary']['onlineCount']}")
...
def main():
    ...
    print("SUPERSONIC 하늘 DOM 자동 수집기")

    with sync_playwright() as p:
        browser = p.chromium.launch_persistent_context(
            user_data_dir=str(BASE_DIR / "chrome_profile_haneul"),
            headless=False,
