...
def team_of(name):
    ...
    if name in {norm(x) for x in NUMBER_TEAM_RIDERS}:
        return "넘버팀"
...
def final_dedupe_riders(riders):
    """
    달서A/B에서 확인된 최종 문제와 동일하게 전화번호 표기 차이(하이픈/공백)를 최종 제거합니다.
    추가로 다른 권역에서 발견된 케이스처럼 같은 이름이 2개인데 한쪽 전화번호만 하이픈 없이 잡힌 경우는
