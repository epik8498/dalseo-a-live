...
def ensure_required_rider_cards(riders):
    ...
    if added:
        debug_log("카드 보강 추가 기사:", ", ".join(added))
    return riders

def _wait_history_rows(page, timeout_seconds=HISTORY_RENDER_TIMEOUT):
    """고정 sleep 대신 DOM 기사행이 안정되는 순간 즉시 반환합니다.

    데이터가 있는 페이지는 행 개수/양끝 식별값이 연속으로 같아지면 준비 완료로 보고,
    마지막 빈 페이지는 짧은 유예 뒤 빈 목록으로 확정합니다.
    """
            ...
            if usable:
                def ident(row):
                    if not isinstance(row, dict):
                        return ""
                    return normalize_phone(row.get("phone", "")) or norm(row.get("userId", "")) or norm(row.get("name", ""))
                signature = (len(rows), ident(usable[0]), ident(usable[-1]))
                if signature == last_signature:
                    stable_hits += 1
                else:
                    last_signature = signature
                    stable_hits = 0
                # 최소 렌더 유예 후 2회 연속 동일하면 즉시 진행
                if elapsed >= 0.35 and stable_hits >= 1:
                    return rows
        elif elapsed >= 0.80:
            # 다음 페이지가 실제 빈 페이지인 경우 불필요하게 networkidle을 기다리지 않습니다.
            return []

        time.sleep(0.12)

    # 제한시간이 지나도 마지막 상태를 한 번 더 읽어 최대한 수집을 이어갑니다.
    try:
...
def change_center(page, config):
    ...
    # 첫 페이지를 미리 열어 collect_all_pages_by_dom의 중복 이동을 없앱니다.
    page.goto(history_url, wait_until="domcontentloaded")
def main():
    print("SUPERSONIC Collector Core v2 - 속도 최적화 / 필수 로그 모드")
    ...
    with sync_playwright() as p:
        ...
        print("배민비즈 로그인/기사실적 확인 후 Enter를 누르세요.")
        input("Enter 대기 중...")

        keep_chrome_rendering(browser, page)
        print("수집 시작 · Chrome은 최소화하지 마세요.")
        ...
        try:
            ...
            while True:
                ...
                print(f"\n[CYCLE {cycle_no}] {cycle_started:%H:%M:%S} · DP {len(CENTER_CONFIGS)}개 · 목표 {CYCLE_TARGET_SECONDS}초")
                success_count = 0
                dp_durations = []

                for idx, config in enumerate(CENTER_CONFIGS, start=1):
                    dp_started_mono = time.monotonic()
                    print(f"[{idx}/{len(CENTER_CONFIGS)}] {config['area']} 수집중...")
                    try:
                        keep_chrome_rendering(browser, page)
                        ...
                        print(f"[{config['area']}] 완료 · {dp_elapsed:.1f}초")
                ...
                print(f"[CYCLE {cycle_no} 완료] {success_count}/{len(CENTER_CONFIGS)} DP 성공")
                print(f"소요 {elapsed:.1f}초 · 사용률 {utilization:.1f}%")
                if avg_dp > 0:
                    print(f"DP 평균 {avg_dp:.1f}초 · 안전 추정 약 {estimated_safe_dp}개")

                if elapsed > safe_budget:
                    print(f"⚠ 용량 경고: 사이클이 {safe_budget:.0f}초(목표의 {CYCLE_WARN_RATIO*100:.0f}%)를 초과했습니다.")
                if elapsed >= CYCLE_TARGET_SECONDS:
                    print("⚠ 3분 목표 초과: 대기 없이 즉시 다음 사이클을 시작합니다. DP 분할 또는 수집 최적화가 필요합니다.")
                else:
                    print(f"대기 {remaining:.1f}초")
