...
def run_update(page, config=None):
    ...
    if not live_path.startswith("/live/maeum"):
        raise RuntimeError(f"업로드 차단: 마음 전용 live 경로가 아닙니다: {live_path}")
    if not weekly_path.startswith("/weekly/maeum"):
...
def change_center(page, config):
    """협력사 선택 박스를 열고 DP코드 기준으로 권역을 안전하게 변경합니다.

    배민비즈 목록이 포털/가상 스크롤 방식으로 늦게 렌더링되는 경우가 있어
    목록 열기와 목표 항목 탐색을 여러 차례 재시도합니다.
    """
    print(f"협력사 변경 시도: {config['area']}")

    target_code = norm(config.get("center_code", ""))
    if not target_code:
        raise RuntimeError(f"{config['area']} center_code가 비어 있습니다.")

    page.goto("https://deliverycenter.baemin.com/center/change")
    page.wait_for_load_state("domcontentloaded")
    time.sleep(1.5)

    def open_center_dropdown():
        return page.evaluate(r"""
        () => {
          const visible = el => {
            const r = el.getBoundingClientRect();
          ...
          const candidates = all.filter(el => {
            ...
            const clickable = tag === 'button' || tag === 'select' || role === 'combobox' ||
                              role === 'button' || el.hasAttribute('aria-haspopup') ||
                              getComputedStyle(el).cursor === 'pointer';
            const nearby = r.top >= pr.bottom - 45 && r.top <= pr.bottom + 300 &&
                           r.right >= pr.left - 180 && r.left <= pr.right + 750;
            return nearby && r.width >= 120 && r.height >= 24 && r.height <= 140 &&
                   (clickable || /DP\d+/.test(t));
          }).sort((a,b) => {
            const ar=a.getBoundingClientRect(), br=b.getBoundingClientRect();
            const ac=(a.getAttribute('role')==='combobox'?1000:0)+
                     ((a.tagName||'').toLowerCase()==='button'?700:0)+
                     (a.hasAttribute('aria-haspopup')?600:0)+
                     (/DP\d+/.test(compact(a.textContent))?500:0)-ar.height;
            const bc=(b.getAttribute('role')==='combobox'?1000:0)+
                     ((b.tagName||'').toLowerCase()==='button'?700:0)+
                     (b.hasAttribute('aria-haspopup')?600:0)+
                     (/DP\d+/.test(compact(b.textContent))?500:0)-br.height;
            return bc-ac || ar.top-br.top;
          });
          ...
          for (const el of candidates) {
            try {
              ...
              el.click();
              return {ok:true, text:(el.textContent||'').replace(/\s+/g,' ').trim().slice(0,160)};
            } catch(e) {}
          }
          return {ok:false, reason:'selector_not_opened'};
        }
        """)

    def select_target_from_open_list():
        return page.evaluate(r"""
        (targetCode) => {
          const visible = el => {
            const r = el.getBoundingClientRect();
          ...
          function chooseTarget() {
            const all = Array.from(document.querySelectorAll('body *')).filter(visible);
            const matches = all.filter(el => compact(el.textContent).includes(targetCode));
            const scored = [];
            ...
            for (const el of matches) {
              ...
              for (let depth=0; depth<7 && cur; depth++, cur=cur.parentElement) {
                ...
                const ct = compact(cur.textContent);
                const dpCodes = ct.match(/DP\d+/g) || [];
                if (!ct.includes(targetCode) || dpCodes.length !== 1) continue;
                if (cr.height > 160 || cr.width > 1100 || cr.height < 18) continue;

                const role = cur.getAttribute('role') || '';
                const tag = (cur.tagName || '').toLowerCase();
                const style = getComputedStyle(cur);
                const clickable = role === 'option' || role === 'button' || tag === 'li' ||
                                  tag === 'button' || cur.hasAttribute('tabindex') ||
                                  style.cursor === 'pointer';
                const score = (clickable ? 6000 : 0) + (role === 'option' ? 3000 : 0) +
                              (tag === 'li' ? 1500 : 0) + (cur.hasAttribute('tabindex') ? 400 : 0) -
                              ct.length - cr.height - depth * 25;
                scored.push({el:cur, score, text:(cur.textContent||'').replace(/\s+/g,' ').trim(), r:cr});
              }
            }

            scored.sort((a,b) => b.score-a.score || a.r.height-b.r.height || a.text.length-b.text.length);
            const seen = new Set();
            for (const c of scored) {
              ...
              try {
                ...
                c.el.dispatchEvent(new MouseEvent('mouseup', {bubbles:true}));
                c.el.click();
                return {ok:true, text:c.text.slice(0,180)};
              } catch(e) {}
            }
            return null;
          }

          let picked = chooseTarget();
          if (picked) return picked;

          // 가상 스크롤/포털 목록을 위에서 아래로 이동하며 목표 항목을 탐색합니다.
          const scrollables = Array.from(document.querySelectorAll('body *')).filter(el => {
          ...
          for (const box of scrollables.slice(0, 8)) {
            try {
              const max = Math.max(0, box.scrollHeight - box.clientHeight);
              const step = Math.max(120, Math.floor(box.clientHeight * 0.7));
          ...
          const visibleDpTexts = Array.from(document.querySelectorAll('body *'))
            .filter(visible)
            .map(el => (el.textContent||'').replace(/\s+/g,' ').trim())
            .filter(t => /DP\d+/.test(t) && t.length < 220);
          return {ok:false, candidates:Array.from(new Set(visibleDpTexts)).slice(0,30)};
        }
        """, target_code)

    opened_text = ""
    selected = {"ok": False, "candidates": []}
    last_open_error = None

    for attempt in range(1, 6):
        opened = open_center_dropdown()
        if not opened.get("ok"):
            last_open_error = opened
            time.sleep(0.7)
            continue

        opened_text = opened.get("text", "")
        if attempt == 1:
            print(f"협력사 드롭다운 열기 완료: {opened_text}")
        else:
            print(f"협력사 목록 재시도 {attempt}/5")

        # 목록 포털 및 가상 스크롤 렌더링 대기
        time.sleep(0.9 + (attempt - 1) * 0.35)
        selected = select_target_from_open_list()
        if selected.get("ok"):
            break

        # 후보가 아예 없으면 목록이 열리지 않았거나 닫힌 것으로 보고 다시 엽니다.
        try:
    ...
    if not selected.get("ok"):
        ...
        try:
            ...
            pass
        raise RuntimeError(
            f"{config['area']}({target_code}) 선택 항목을 5회 재시도했지만 찾지 못했습니다. "
            f"후보: {selected.get('candidates', [])} / 열기오류: {last_open_error} / 캡처: {shot.name}"
        )

    print(f"협력사 항목 선택 완료: {config['area']} / {selected.get('text', '')}")
    time.sleep(0.6)
    ...
    try:
        ...
        raise RuntimeError(f"{config['area']} 선택 후 화면에서 {target_code}를 확인하지 못했습니다.")
    ...
    current_ok = page.evaluate(r"""
    (targetCode) => {
      const text = (document.body.innerText || '').replace(/\s+/g, ' ');
      return text.slice(0, 2200).includes(targetCode);
    }
    """, target_code)
    if not current_ok:
        shot = BASE_DIR / f"center_verify_fail_{config['slug']}.png"
        try:
            page.screenshot(path=str(shot), full_page=True)
        except Exception:
            pass
        raise RuntimeError(f"협력사 전환 검증 실패: {config['area']} / {target_code} / 캡처: {shot.name}")

    global VERIFIED_CENTER_CODE
    VERIFIED_CENTER_CODE = target_code
    print(f"협력사 변경 완료 및 검증 성공: {config['area']} / {target_code}")

def main():
    print("MAEUM 안전 독립 다권역 DOM 자동 수집기 v3")
