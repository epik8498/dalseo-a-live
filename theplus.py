...
DATA_FILE = BASE_DIR / "data_theplus.json"
HTML_FILE = BASE_DIR / "theplus.html"
WEEKLY_FILE = BASE_DIR / "weekly_theplus.json"
...
CENTER_CONFIGS = [{
    ...
    'slug': 'theplus',
    'aliases': [
        '표준대구중A주식회사바이럴커넥티드(DP2509099587)',
        '표준대구중A주식회사바이럴커넥티드 (DP2509099587)',
        '표준대구중A주식회사바이럴커넥티드',
        'DP2509099587'
    ],
...
def git_push():
    ...
    git_files = [DATA_FILE.name, HTML_FILE.name, Path(__file__).name, "logo.png"]
    subprocess.run(["git", "add", *git_files], cwd=BASE_DIR)

    if WEEKLY_FILE.exists():
        subprocess.run(["git", "add", WEEKLY_FILE.name], cwd=BASE_DIR)
...
def change_center(page, config):
    ...
    time.sleep(1.5)
def main():
    print("THE+ 전용 DOM 자동 수집기 - 화면 밖 백그라운드 모드")
    print("대상 권역:", ", ".join(c["area"] for c in CENTER_CONFIGS))

    with sync_playwright() as p:
        browser = p.chromium.launch_persistent_context(
            user_data_dir=str(BASE_DIR / "chrome_profile_theplus"),
            headless=False,
        ...
        try:
            while True:
                ...
                print(f"{REFRESH_SECONDS}초 후 THE+ 센터를 다시 수집합니다.")
