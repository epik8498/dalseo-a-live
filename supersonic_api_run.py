...
# PROD/DEV는 DB, 인증 Secret, MFA 키, 백업 폴더를 서로 공유하지 않습니다.
# 기존 운영 BAT의 SUPERSONIC_PRODUCTION=1은 현재 운영 파일명을 그대로 사용합니다.
ENV_NAME = "PROD" if os.getenv("SUPERSONIC_PRODUCTION", "").strip() == "1" else "DEV"
IS_PRODUCTION = ENV_NAME == "PROD"

CONFIG_PATH = BASE_DIR / ("supersonic_enterprise_config_v1.json" if IS_PRODUCTION else "supersonic_enterprise_config_dev.json")
DB_PATH = BASE_DIR / ("supersonic_enterprise_v1.db" if IS_PRODUCTION else "supersonic_enterprise_dev.db")
BACKUP_DIR = BASE_DIR / ("backups" if IS_PRODUCTION else "backups_dev")
BACKUP_RETENTION_DAYS = 30
BACKUP_HOUR = 3
BACKUP_MINUTE = 10

LOCAL_APPDATA = Path(os.getenv("LOCALAPPDATA", str(BASE_DIR)))
PROD_SECRETS_DIR = LOCAL_APPDATA / "SUPERSONIC" / "secrets"
if IS_PRODUCTION:
    SECRET_PATH = PROD_SECRETS_DIR / ".supersonic_api_secret"
    MFA_KEY_PATH = PROD_SECRETS_DIR / ".supersonic_mfa_key"
else:
    SECRET_PATH = BASE_DIR / ".supersonic_api_secret_dev"
    MFA_KEY_PATH = BASE_DIR / ".supersonic_mfa_key_dev"

MFA_ENC_PREFIX = "enc:v1:"
MFA_ENC_AAD = b"SUPERSONIC-MFA-v1"
...
ADMIN_HTML_PATH = BASE_DIR / "supersonic_enterprise_admin_v3_2_계정상태관리.html"
...
def ensure_secret() -> bytes:
    ...
    if IS_PRODUCTION:
        raise RuntimeError(f"운영 인증 Secret이 없습니다: {SECRET_PATH}")
...
def init_db() -> None:
    with db_connect() as conn:
        ...
        if "account_limit" not in user_cols:
            conn.execute("ALTER TABLE users ADD COLUMN account_limit INTEGER")
        ...
        if "remember_login" not in session_cols:
            conn.execute("ALTER TABLE sessions ADD COLUMN remember_login INTEGER NOT NULL DEFAULT 0")

        challenge_cols = {row["name"] for row in conn.execute("PRAGMA table_info(mfa_challenges)").fetchall()}
        if "remember_login" not in challenge_cols:
            conn.execute("ALTER TABLE mfa_challenges ADD COLUMN remember_login INTEGER NOT NULL DEFAULT 0")

        conn.execute(
            "INSERT OR IGNORE INTO tenants(id,name,active,created_at) VALUES(?,?,1,?)",
            ("internal", "SUPERSONIC Internal", iso_utc()),
        )
        migrated_mfa = migrate_plaintext_mfa_secrets(conn)
        conn.commit()
...
def verify_sqlite_file(path: Path) -> None:
    if not path.exists() or path.stat().st_size <= 0:
        raise RuntimeError(f"백업 파일이 없거나 비어 있습니다: {path}")
    ...
    try:
        ...
        required = {"tenants", "users", "center_permissions", "sessions", "audit_logs"}
        tables = {r[0] for r in conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table'"
        ).fetchall()}
        missing = sorted(required - tables)
        if missing:
            raise RuntimeError("필수 테이블 누락: " + ", ".join(missing))
    finally:
        conn.close()


def create_db_backup(reason: str = "manual") -> Path:
    """실행 중인 WAL DB도 SQLite Backup API로 일관성 있게 백업합니다."""
    init_db()
    BACKUP_DIR.mkdir(parents=True, exist_ok=True)
    safe_reason = re.sub(r"[^A-Za-z0-9_-]+", "_", reason or "manual")[:24]
    target = BACKUP_DIR / f"supersonic_{_backup_timestamp()}_{safe_reason}.db"
    source_conn = sqlite3.connect(DB_PATH, timeout=30)
    backup_conn = sqlite3.connect(target, timeout=30)
    try:
...
def cleanup_old_backups() -> None:
    ...
    for path in BACKUP_DIR.glob("supersonic_*.db"):
        try:
...
def list_db_backups() -> list[Path]:
    BACKUP_DIR.mkdir(parents=True, exist_ok=True)
    return sorted(BACKUP_DIR.glob("supersonic_*.db"), key=lambda p: p.stat().st_mtime, reverse=True)


def restore_db_backup(backup_name: str) -> None:
    """
    지정 백업을 운영 DB로 복구합니다.
    복구 전 현재 운영 DB를 별도 안전백업한 뒤,
    Windows에서 -wal/-shm 파일을 직접 삭제하지 않고 SQLite Backup API로
    기존 운영 DB 파일 안에 백업 내용을 복원합니다.
    API 서버는 종료한 상태에서 실행하는 것을 원칙으로 합니다.
    """
    name = Path(str(backup_name)).name
    source = BACKUP_DIR / name
    if not source.exists():
        raise FileNotFoundError(f"백업 파일을 찾을 수 없습니다: {name}")
    verify_sqlite_file(source)

    # 복구 직전 현재 상태를 반드시 별도 보존합니다.
    safety = create_db_backup("before_restore")
    ...
    try:
        ...
        # 파일 교체/os.replace 및 -wal/-shm 강제 삭제를 하지 않습니다.
        # SQLite 자체 backup API가 목적 DB에 일관된 페이지 단위로 복원합니다.
        ...
        result = str(row[0] if row else "")
        if result.lower() != "ok":
            raise RuntimeError(f"복구 후 SQLite 무결성 검사 실패: {result}")
    finally:
        dst_conn.close()
        src_conn.close()

    verify_sqlite_file(DB_PATH)
    print(f"복구 완료: {name}")
    print(f"복구 직전 안전백업: {safety.name}")

def automatic_backup_loop(stop_event: threading.Event) -> None:
    """API 실행 중 매일 03:10 로컬시간에 1회 자동 백업합니다."""
    last_date = ""
    while not stop_event.wait(30):
        now = datetime.now()
        today = now.strftime("%Y-%m-%d")
        if now.hour == BACKUP_HOUR and now.minute >= BACKUP_MINUTE and last_date != today:
            try:
                path = create_db_backup("daily")
                last_date = today
                print(f"DB 자동백업 완료: {path.name}")
...
def _load_or_create_mfa_encryption_key() -> bytes:
    """MFA DB 암호화 전용 256-bit 키. DB와 분리된 로컬 비밀파일에 저장합니다."""
    if MFA_KEY_PATH.exists():
        raw = MFA_KEY_PATH.read_text(encoding="utf-8").strip()
        try:
            key = base64.urlsafe_b64decode(raw.encode("ascii"))
        except Exception as exc:
            raise RuntimeError("MFA 암호화 키 파일을 읽을 수 없습니다.") from exc
        if len(key) != 32:
            raise RuntimeError("MFA 암호화 키 길이가 올바르지 않습니다.")
        return key

    if IS_PRODUCTION:
        raise RuntimeError(f"운영 MFA 암호화 키가 없습니다: {MFA_KEY_PATH}")

    key = secrets.token_bytes(32)
    encoded = base64.urlsafe_b64encode(key).decode("ascii")
    # DEV에서만 새 파일을 생성합니다. 기존 키를 절대 덮어쓰지 않습니다.
    try:
        with open(MFA_KEY_PATH, "x", encoding="utf-8") as f:
            f.write(encoded)
        try:
        ...
        return _load_or_create_mfa_encryption_key()


def encrypt_mfa_secret(secret: str | None) -> str | None:
    if not secret:
        return None
    value = str(secret)
    if value.startswith(MFA_ENC_PREFIX):
        return value
    key = _load_or_create_mfa_encryption_key()
    nonce = secrets.token_bytes(12)
    ciphertext = AESGCM(key).encrypt(nonce, value.encode("utf-8"), MFA_ENC_AAD)
    blob = base64.urlsafe_b64encode(nonce + ciphertext).decode("ascii")
    return MFA_ENC_PREFIX + blob


def decrypt_mfa_secret(value: str | None) -> str:
    if not value:
        return ""
    text = str(value)
    # v3.4 전환 중 기존 평문 레코드도 읽을 수 있게 하되 init_db에서 즉시 암호화합니다.
    if not text.startswith(MFA_ENC_PREFIX):
        return text
    try:
        raw = base64.urlsafe_b64decode(text[len(MFA_ENC_PREFIX):].encode("ascii"))
        if len(raw) < 13:
            raise ValueError("ciphertext too short")
        nonce, ciphertext = raw[:12], raw[12:]
        key = _load_or_create_mfa_encryption_key()
        return AESGCM(key).decrypt(nonce, ciphertext, MFA_ENC_AAD).decode("utf-8")
    except Exception as exc:
        raise RuntimeError("MFA Secret 복호화 실패: 암호화 키 파일을 확인하세요.") from exc


def migrate_plaintext_mfa_secrets(conn: sqlite3.Connection) -> int:
    """기존 평문 MFA Secret을 같은 트랜잭션에서 AES-256-GCM 암호문으로 전환합니다."""
    rows = conn.execute(
        "SELECT id,mfa_secret FROM users WHERE mfa_secret IS NOT NULL AND mfa_secret<>''"
    ).fetchall()
    changed = 0
    for row in rows:
        value = str(row["mfa_secret"] or "")
        if value and not value.startswith(MFA_ENC_PREFIX):
            conn.execute(
                "UPDATE users SET mfa_secret=? WHERE id=?",
                (encrypt_mfa_secret(value), row["id"]),
            )
            changed += 1
    return changed
...
def setup_user_mfa(username: str) -> None:
    ...
    with db_connect() as conn:
        conn.execute(
            ...
            (encrypt_mfa_secret(secret), user["id"]),
        )
...
def create_mfa_challenge(user: sqlite3.Row, ip: str, remember_login: bool = False) -> str:
    ...
    with db_connect() as conn:
        ...
        conn.execute("DELETE FROM mfa_challenges WHERE user_id=?", (user["id"],))
        conn.execute(
            "INSERT INTO mfa_challenges(token_hash,user_id,created_at,expires_at,ip,attempts,remember_login) "
            "VALUES(?,?,?,?,?,0,?)",
            (th, user["id"], iso_utc(), iso_utc(expires), ip, 1 if remember_login else 0),
        )
        conn.commit()
    return raw


def issue_session(user: sqlite3.Row, remember_login: bool = False) -> tuple[str, int]:
    token = secrets.token_urlsafe(40)
    th = token_hash(token)
    max_age = REMEMBER_SESSION_DAYS * 86400 if remember_login else SESSION_ABSOLUTE_HOURS * 3600
    expires = now_utc() + timedelta(seconds=max_age)
    with db_connect() as conn:
        conn.execute("DELETE FROM sessions WHERE expires_at<=?", (iso_utc(),))
        # 지점/조회 계정은 동시 로그인 1대만 허용. 새 로그인 시 이전 기기 세션 종료.
        if user["role"] in {ROLE_BRANCH_ADMIN, ROLE_VIEWER}:
            conn.execute("DELETE FROM sessions WHERE user_id=?", (user["id"],))
        conn.execute(
            "INSERT INTO sessions(token_hash,user_id,created_at,expires_at,last_seen_at,remember_login) "
            "VALUES(?,?,?,?,?,?)",
            (th, user["id"], iso_utc(), iso_utc(expires), iso_utc(), 1 if remember_login else 0),
        )
        conn.commit()
...
def login_lock_summary(username: str) -> dict[str, Any]:
    cutoff = now_utc() - timedelta(minutes=LOGIN_LOCK_MINUTES)
    with db_connect() as conn:
        rows = conn.execute(
            "SELECT ip,COUNT(*) AS cnt,MAX(failed_at) AS last_failed "
            "FROM login_failures WHERE username=? COLLATE NOCASE AND failed_at>=? "
            "GROUP BY ip",
            (username, iso_utc(cutoff)),
        ).fetchall()
    ...
    for row in rows:
        ...
        failed_count = max(failed_count, cnt)
        if cnt >= LOGIN_MAX_FAILURES:
            locked = True
            last_failed = datetime.fromisoformat(str(row["last_failed"]))
            unlock_at = last_failed + timedelta(minutes=LOGIN_LOCK_MINUTES)
            retry_after = max(retry_after, max(0, int((unlock_at - now).total_seconds())))
    return {
        "locked": locked,
...
def ensure_firebase() -> None:
    ...
    # 개발환경은 명시적으로 허용하지 않는 한 Firebase 접근 자체를 차단합니다.
    # 실수로 운영 RTDB를 읽거나 수정하는 사고를 코드 레벨에서 방지합니다.
    if not IS_PRODUCTION and os.getenv("SUPERSONIC_DEV_FIREBASE", "").strip() != "1":
        raise RuntimeError("DEV_FIREBASE_DISABLED")

    with _FIREBASE_LOCK:
        if not _FIREBASE_READY:
            from firebase_uploader import init_firebase
            init_firebase(environment=ENV_NAME)
...
def load_user_by_session(token: str) -> sqlite3.Row | None:
    ...
    with db_connect() as conn:
        session = conn.execute(
            ...
            "WHERE s.token_hash=? AND u.active=1 AND t.active=1",
            (th,),
        ).fetchone()
        ...
        # 일반 로그인은 기존 30분 미사용 제한, 자동 로그인은 30일 만료까지 유지.
        remembered = bool(session["remember_login"]) if "remember_login" in session.keys() else False
        if session["expires_at"] <= now or (not remembered and session["last_seen_at"] <= idle_cutoff):
...
def client_ip(handler: BaseHTTPRequestHandler) -> str:
    # 운영은 Cloudflare Tunnel 뒤에서 사용하므로 Cloudflare가 제공하는 원본 IP를 우선합니다.
    # 임의 X-Forwarded-For만으로 로그인 잠금/IP 감사로그를 우회하는 위험을 줄입니다.
    if IS_PRODUCTION:
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def _require_admin_manager(self) -> sqlite3.Row | None:
        ...
        if not user:
            return None
        if user["role"] not in {ROLE_SUPER_ADMIN, ROLE_HQ_ADMIN, ROLE_BRANCH_ADMIN}:
    ...
    def do_GET(self) -> None:
        ...
        if path == "/api/admin/overview":
            ...
            is_hq = user["role"] == ROLE_HQ_ADMIN
            is_branch = user["role"] == ROLE_BRANCH_ADMIN
            with db_connect() as conn:
                if is_super:
                    tenant_rows = conn.execute("SELECT id,name,active,created_at FROM tenants ORDER BY created_at,id").fetchall()
                    user_rows = conn.execute("SELECT id,tenant_id,username,role,active,created_at,account_limit,created_by_user_id FROM users ORDER BY created_at,id").fetchall()
                elif is_hq:
                    tenant_rows = conn.execute("SELECT id,name,active,created_at FROM tenants WHERE id=?", (user["tenant_id"],)).fetchall()
                    user_rows = conn.execute(
                        "SELECT id,tenant_id,username,role,active,created_at,account_limit,created_by_user_id FROM users "
                        "WHERE tenant_id=? AND role IN (?,?) ORDER BY created_at,id",
                        (user["tenant_id"], ROLE_BRANCH_ADMIN, ROLE_VIEWER)).fetchall()
                else:
                    tenant_rows = conn.execute("SELECT id,name,active,created_at FROM tenants WHERE id=?", (user["tenant_id"],)).fetchall()
                    user_rows = conn.execute(
                        "SELECT id,tenant_id,username,role,active,created_at,account_limit,created_by_user_id FROM users "
                        "WHERE id=? OR created_by_user_id=? ORDER BY created_at,id",
                ...
                if user_ids:
                    ...
                    perm_rows=conn.execute(
                        ...
                        f"WHERE user_id IN ({marks}) ORDER BY user_id,center_slug", user_ids).fetchall()
                else: perm_rows=[]

                account_used_by_creator = {}
                if user_ids:
                    usage_rows = conn.execute(
                        "SELECT created_by_user_id,COUNT(*) AS c FROM users "
                        f"WHERE created_by_user_id IN ({marks}) GROUP BY created_by_user_id",
                        user_ids,
                    ).fetchall()
                    account_used_by_creator = {r["created_by_user_id"]: int(r["c"] or 0) for r in usage_rows}

                branch_used = 0
                if is_branch:
                    branch_used = int(conn.execute(
                        "SELECT COUNT(*) AS c FROM users WHERE created_by_user_id=?",
                        (user["id"],)).fetchone()["c"] or 0)
            ...
            for row in user_rows:
                item=dict(row); item["permissions"]=perms_by_user.get(row["id"],[])
                item["account_used"] = account_used_by_creator.get(row["id"], 0) if row["role"] == ROLE_BRANCH_ADMIN else None
                item["isCurrent"]=row["id"]==user["id"]
                lock_info=login_lock_summary(row["username"])
                item["loginLocked"]=bool(lock_info["locked"]); item["loginFailedCount"]=int(lock_info["failedCount"])
                item["loginRetryAfterSeconds"]=int(lock_info["retryAfterSeconds"])
                if is_super: item["manageable"]=row["id"]!=user["id"]
                elif is_hq: item["manageable"]=row["id"]!=user["id"] and row["tenant_id"]==user["tenant_id"] and row["role"] in {ROLE_BRANCH_ADMIN,ROLE_VIEWER}
                else: item["manageable"]=row["created_by_user_id"]==user["id"] and row["role"]==ROLE_VIEWER
                users.append(item)

            if is_super:
                visible_centers=CENTERS; roles=sorted(VALID_ROLES)
            else:
                my_perms=list_permissions(user)
                visible_centers={slug:center for slug,center in CENTERS.items() if my_perms.get(slug,{}).get("can_view")}
                roles=[ROLE_BRANCH_ADMIN,ROLE_VIEWER] if is_hq else [ROLE_VIEWER]

            centers=[{"slug":slug,"name":center.get("name") or slug,"dpCode":center.get("dp_code") or ""} for slug,center in visible_centers.items()]
            self._json(HTTPStatus.OK,{
                "ok":True,"currentUserId":user["id"],
                "adminMode":"SUPER" if is_super else ("HQ" if is_hq else "BRANCH"),
                "canCreateTenant":is_super,"currentTenantId":user["tenant_id"],
                "accountLimit": user["account_limit"] if "account_limit" in user.keys() else None,
                "accountUsed": branch_used if is_branch else None,
                "tenants":[dict(r) for r in tenant_rows],"users":users,"centers":centers,"roles":roles})
            return
        ...
        if path == "/api/audit":
            ...
            try:
                ...
                self._json(HTTPStatus.BAD_REQUEST, {"ok": False, "error": "INVALID_LIMIT"})
                return
            ...
            if user["role"] == ROLE_SUPER_ADMIN:
                ...
                # HQ는 자기 tenant 범위의 감사로그만 볼 수 있습니다.
                logs = [audit_public_item(x) for x in read_audit_rows_for_monitor(user, limit)]

            self._json(HTTPStatus.OK, {"ok": True, "logs": logs})
            return
    ...
    def do_POST(self) -> None:
        ...
        # 로그인/MFA를 제외한 상태변경 요청이 브라우저 쿠키 인증을 쓰는 경우
        # 허용된 Origin을 반드시 요구해 CSRF 방어를 SameSite 쿠키와 이중화합니다.
        if path not in {"/api/auth/login", "/api/auth/mfa"}:
            auth = self.headers.get("Authorization", "")
            cookie_header = self.headers.get("Cookie", "")
            uses_cookie_session = COOKIE_NAME in cookie_header and not auth.lower().startswith("bearer ")
            if uses_cookie_session and not self.headers.get("Origin", "").strip():
                self._json(HTTPStatus.FORBIDDEN, {"ok": False, "error": "ORIGIN_REQUIRED"})
                return
        ...
        if path == "/api/auth/login":
            ...
            remember_login = bool(body.get("remember", False))
            ip = client_ip(self)
            ...
            if user["role"] in MFA_REQUIRED_ROLES:
                if not int(user["mfa_enabled"] or 0) or not user["mfa_secret"]:
                    ...
                    return
                challenge = create_mfa_challenge(user, ip, remember_login)
            ...
            token, max_age = issue_session(user, remember_login)
            audit("LOGIN_SUCCESS", user, None, ip, {"mfa": False})
            cookie = session_cookie(token, max_age)
            self._json(
                HTTPStatus.OK,
        ...
        if path == "/api/auth/mfa":
            ...
            with db_connect() as conn:
                row = conn.execute(
                    "SELECT c.token_hash,c.user_id,c.expires_at,c.ip,c.attempts,c.remember_login,u.* "
            ...
            matched_counter = verify_totp(
                decrypt_mfa_secret(row["mfa_secret"]),
                code,
            ...
            token, max_age = issue_session(row, bool(row["remember_login"]))
        ...
        if path == "/api/admin/user/create":
            ...
            requested_account_limit = body.get("accountLimit", None)

            if user["role"] in {ROLE_HQ_ADMIN, ROLE_BRANCH_ADMIN}:
                tenant_id = user["tenant_id"]
            if user["role"] == ROLE_BRANCH_ADMIN:
                role = ROLE_VIEWER
            ...
            new_account_limit = None
            if role == ROLE_BRANCH_ADMIN:
                if user["role"] not in {ROLE_SUPER_ADMIN, ROLE_HQ_ADMIN}:
                    self._json(HTTPStatus.FORBIDDEN, {"ok": False, "error": "ROLE_SCOPE_DENIED"})
                    return
                if requested_account_limit in {None, ""}:
                    self._json(HTTPStatus.BAD_REQUEST, {
                        "ok": False,
                        "error": "ACCOUNT_LIMIT_REQUIRED",
                        "message": "지점 계정을 발급할 때 관리자 계정 발급 한도를 지정해야 합니다.",
                    })
                    return
                try:
                    new_account_limit = int(requested_account_limit)
                except (TypeError, ValueError):
                    self._json(HTTPStatus.BAD_REQUEST, {"ok": False, "error": "INVALID_ACCOUNT_LIMIT"})
                    return
                if new_account_limit < 0 or new_account_limit > 9999:
                    self._json(HTTPStatus.BAD_REQUEST, {
                        "ok": False,
                        "error": "INVALID_ACCOUNT_LIMIT",
                        "message": "계정 발급 한도는 0~9999개 사이로 입력하세요.",
                    })
                    return
            ...
            if user["role"] == ROLE_BRANCH_ADMIN:
                own = list_permissions(user)
                feature_to_field = {"view":"can_view","weekly":"can_view_weekly","settlement":"can_view_settlement",
                                    "personal":"can_view_personal","manage":"can_manage_riders","sms":"can_send_sms"}
                for slug in center_slugs:
                    p = own.get(slug)
                    if not p or not p.get("can_view"):
                        self._json(HTTPStatus.FORBIDDEN, {"ok":False,"error":"CENTER_SCOPE_DENIED"}); return
                    for feature in feature_names:
                        field=feature_to_field.get(feature)
                        if field and not p.get(field):
                            self._json(HTTPStatus.FORBIDDEN, {"ok":False,"error":"FEATURE_SCOPE_DENIED"}); return
                limit = user["account_limit"] if "account_limit" in user.keys() else None
                if limit is None:
                    self._json(HTTPStatus.FORBIDDEN, {"ok":False,"error":"ACCOUNT_LIMIT_NOT_ASSIGNED",
                                                     "message":"개발자 또는 중개사가 관리자 계정 발급 한도를 먼저 지정해야 합니다."}); return
                with db_connect() as conn:
                    used=int(conn.execute("SELECT COUNT(*) AS c FROM users WHERE created_by_user_id=?",(user["id"],)).fetchone()["c"] or 0)
                if used >= int(limit):
                    self._json(HTTPStatus.CONFLICT, {"ok":False,"error":"ACCOUNT_LIMIT_REACHED",
                                                    "message":f"계정 발급 한도 {limit}개를 모두 사용했습니다."}); return
            ...
            try:
                with db_connect() as conn:
                    ...
                    if not tenant:
                        ...
                        return
                    conn.execute(
                        "INSERT INTO users(id,tenant_id,username,password_salt,password_hash,role,active,created_at,created_by_user_id,account_limit) "
                        "VALUES(?,?,?,?,?,?,1,?,?,?)",
                        (new_user_id, tenant_id, username, salt_b64, hash_b64, role, iso_utc(), user["id"], new_account_limit),
                    )
                    for slug in center_slugs:
                        values = {field: 0 for field in PERMISSION_FIELDS}
                        for feature in feature_names:
            ...
            audit("ADMIN_USER_CREATE", user, None, client_ip(self),
                  {"targetUserId":new_user_id,"username":username,"tenantId":tenant_id,
                   ...
                   "accountLimit":new_account_limit})
        ...
        if path == "/api/admin/user/update":
            ...
            with db_connect() as conn:
                target = conn.execute(
                    "SELECT id,tenant_id,role,created_by_user_id FROM users WHERE id=?",
                    (target_id,),
                ).fetchone()
            if not target:
            ...
            if user["role"] == ROLE_BRANCH_ADMIN:
                if target["role"] != ROLE_VIEWER or ("created_by_user_id" in target.keys() and target["created_by_user_id"] != user["id"]):
                    self._json(HTTPStatus.FORBIDDEN, {"ok": False, "error": "TARGET_SCOPE_DENIED"})
                    return
            if user["role"] == ROLE_HQ_ADMIN:
            ...
            audit(
                ...
                {"targetUserId":target_id,"tenantId":tenant_id,"role":role,"active":active,
                 "centers":center_slugs,"features":sorted(feature_names),"sessionsRevoked":True},
            )
            self._json(HTTPStatus.OK, {"ok": True})
            return

        if path == "/api/admin/user/set-active":
            if user["role"] not in {ROLE_SUPER_ADMIN, ROLE_HQ_ADMIN, ROLE_BRANCH_ADMIN}:
                self._json(HTTPStatus.FORBIDDEN, {"ok": False, "error": "ADMIN_MANAGER_REQUIRED"})
                return
            target_id = str(body.get("userId", "")).strip()
            ...
            with db_connect() as conn:
                target = conn.execute(
                    "SELECT id,tenant_id,username,role,active FROM users WHERE id=?",
                    (target_id,),
                ).fetchone()
            if not target:
                self._json(HTTPStatus.NOT_FOUND, {"ok": False, "error": "USER_NOT_FOUND"})
            ...
            audit(
                "ADMIN_USER_ACTIVATE" if active else "ADMIN_USER_DEACTIVATE",
                user,
                None,
                client_ip(self),
                {"targetUserId": target_id, "username": target["username"], "sessionsRevoked": True},
            )
            self._json(HTTPStatus.OK, {"ok": True, "active": bool(active)})
            return

        if path == "/api/admin/user/set-account-limit":
            if user["role"] not in {ROLE_SUPER_ADMIN, ROLE_HQ_ADMIN}:
                self._json(HTTPStatus.FORBIDDEN, {"ok":False,"error":"ADMIN_MANAGER_REQUIRED"}); return
            target_id=str(body.get("userId","")).strip()
            try: limit=int(body.get("limit"))
            except (TypeError,ValueError):
                self._json(HTTPStatus.BAD_REQUEST, {"ok":False,"error":"INVALID_ACCOUNT_LIMIT"}); return
            if limit < 0 or limit > 9999:
                self._json(HTTPStatus.BAD_REQUEST, {"ok":False,"error":"INVALID_ACCOUNT_LIMIT",
                                                    "message":"계정 발급 한도는 0~9999개 사이로 입력하세요."}); return
            with db_connect() as conn:
                target=conn.execute("SELECT id,tenant_id,username,role,created_by_user_id FROM users WHERE id=?",(target_id,)).fetchone()
                if not target:
                    self._json(HTTPStatus.NOT_FOUND, {"ok":False,"error":"USER_NOT_FOUND"}); return
                if target["role"] != ROLE_BRANCH_ADMIN:
                    self._json(HTTPStatus.BAD_REQUEST, {"ok":False,"error":"BRANCH_ADMIN_REQUIRED"}); return
                if user["role"]==ROLE_HQ_ADMIN and target["tenant_id"]!=user["tenant_id"]:
                    self._json(HTTPStatus.FORBIDDEN, {"ok":False,"error":"TENANT_SCOPE_DENIED"}); return
                used=int(conn.execute("SELECT COUNT(*) AS c FROM users WHERE created_by_user_id=?",(target_id,)).fetchone()["c"] or 0)
                if limit < used:
                    self._json(HTTPStatus.CONFLICT, {"ok":False,"error":"LIMIT_BELOW_CURRENT_USAGE",
                                                    "message":f"현재 {used}개 계정이 있어 한도를 {used}보다 작게 설정할 수 없습니다."}); return
                conn.execute("UPDATE users SET account_limit=? WHERE id=?",(limit,target_id)); conn.commit()
            audit("ADMIN_ACCOUNT_LIMIT_UPDATE",user,None,client_ip(self),
                  {"targetUserId":target_id,"username":target["username"],"limit":limit,"used":used})
            self._json(HTTPStatus.OK, {"ok":True,"limit":limit,"used":used}); return
        ...
        if path == "/api/admin/user/delete":
            ...
            target_id = str(body.get("userId", "")).strip()
            if not target_id:
                self._json(HTTPStatus.BAD_REQUEST, {"ok": False, "error": "USER_ID_REQUIRED"})
                return
            if target_id == user["id"]:
                self._json(HTTPStatus.BAD_REQUEST, {"ok": False, "error": "CURRENT_ADMIN_PROTECTED"})
                return
            ...
            try:
                with db_connect() as conn:
                    target = conn.execute(
                        "SELECT id,tenant_id,username,role,created_by_user_id FROM users WHERE id=?",
                        (target_id,),
                    ).fetchone()
                    ...
                    # FK CASCADE에만 의존하지 않고 관련 로그인/권한 데이터를 명시적으로 정리.
                    conn.execute("DELETE FROM center_permissions WHERE user_id=?", (target_id,))
                    conn.execute("DELETE FROM sessions WHERE user_id=?", (target_id,))
                    conn.execute("DELETE FROM mfa_challenges WHERE user_id=?", (target_id,))
                    conn.execute("DELETE FROM login_failures WHERE username=? COLLATE NOCASE", (target["username"],))
                    cur = conn.execute("DELETE FROM users WHERE id=?", (target_id,))
                    if cur.rowcount != 1:
                        raise RuntimeError("USER_DELETE_NOT_APPLIED")
                    conn.commit()

                # 삭제가 실제 반영된 뒤 감사로그 기록.
                audit(
                    "ADMIN_USER_DELETE",
                    user,
                    None,
                    client_ip(self),
                    {"targetUserId": target_id, "username": target["username"],
                     "tenantId": target["tenant_id"], "role": target["role"]},
                )
                self._json(HTTPStatus.OK, {"ok": True, "deletedUserId": target_id})
                return
            except Exception as exc:
                print(f"[ADMIN USER DELETE 오류] {type(exc).__name__}: {exc}")
                self._json(
                    HTTPStatus.INTERNAL_SERVER_ERROR,
                    {"ok": False, "error": "USER_DELETE_FAILED", "message": str(exc)},
                )
                return

        if path == "/api/admin/user/unlock-login":
            if user["role"] not in {ROLE_SUPER_ADMIN, ROLE_HQ_ADMIN, ROLE_BRANCH_ADMIN}:
                self._json(HTTPStatus.FORBIDDEN, {"ok": False, "error": "ADMIN_MANAGER_REQUIRED"})
                return
            target_id = str(body.get("userId", "")).strip()
            ...
            audit(
                ...
                None,
                client_ip(self),
                {"targetUserId": target_id, "username": target["username"], "deletedFailures": cur.rowcount},
            )
            self._json(HTTPStatus.OK, {"ok": True, "deletedFailures": cur.rowcount})
            return
...
def serve() -> None:
    ...
    port = int(SERVICE.get("api_port", 8787) if IS_PRODUCTION else SERVICE.get("api_port_dev", 8788))
    server = ThreadingHTTPServer((host, port), ApiHandler)
    backup_stop = threading.Event()
    backup_thread = threading.Thread(
        target=automatic_backup_loop,
        args=(backup_stop,),
        name="supersonic-db-backup",
        daemon=True,
    )
    backup_thread.start()
    print("SUPERSONIC Enterprise API Core v3.8")
    print(f"환경: {ENV_NAME} · DB: {DB_PATH.name}")
    if IS_PRODUCTION:
        print(f"Secrets: {PROD_SECRETS_DIR}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SUPERSONIC Enterprise API Core v3.8")
    ...
    parser.add_argument("--backup", action="store_true", help="DB 즉시 안전백업")
    parser.add_argument("--list-backups", action="store_true", help="DB 백업 목록")
    parser.add_argument("--restore", metavar="BACKUP_FILE", help="지정 백업으로 DB 복구 (API 종료 후 실행)")
    ...
    if args.list_backups:
        ...
        return
    if args.restore:
        restore_db_backup(args.restore)
        return
    if args.check:
        with db_connect() as conn:
            user_count = conn.execute("SELECT COUNT(*) FROM users").fetchone()[0]
        print(f"설정 정상 · 환경 {ENV_NAME} · DP {len(CENTERS)}개 · 사용자 {user_count}명 · DB {DB_PATH.name}")
        return
    serve()
