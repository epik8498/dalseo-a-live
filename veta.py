...
DATA_FILE = BASE_DIR / "data_veta.json"
HTML_FILE = BASE_DIR / "veta.html"
WEEKLY_FILE = BASE_DIR / "weekly_veta.json"
...
CENTER_CONFIGS = [{
    ...
    'customer_name': 'VETA',
    'slug': 'veta',
    # VETA 경산B 실제 센터 DP코드를 고정합니다.
    # 다른 협력사로 잘못 전환되는 것을 막기 위해 DP코드로 엄격하게 검증합니다.
    'center_code': 'DP2508085156',
    'center_search': '경산B',
    'aliases': ['경산B', 'VETA', 'DP2508085156'],
    'team_order': ['팀1', '팀2', '팀3'],
    # 테스트 운영용 동일 가중치. 실제 세트 수가 확정되면 숫자만 변경하면 됩니다.
    'area_config': {'팀1': 1.0, '팀2': 1.0, '팀3': 1.0},
    'team_map_path': '/settings/veta/teamMap',
    'live_path': '/live/veta',
    'weekly_path': '/weekly/veta',
    'required_team_riders': {},
    'test_split_counts': {'팀1': 20, '팀2': 20, '팀3': 19},
}]
...
def resolve_team_safely(rider, duplicate_names=None):
    ...
    if name and name not in duplicate_names and CURRENT_SLUG != "veta":
...
def apply_veta_test_split(riders):
    """VETA 테스트용 59명 자동 분배.

    최초 배정은 기사명을 가나다순(동명이인은 stable ID 보조)으로 정렬해
    팀1 20명 / 팀2 20명 / 팀3 19명으로 나눕니다.
    한 번 Firebase에 저장된 stable ID 배정은 이후 기사관리 변경을 포함해 우선 보존합니다.
    이름은 배정 키로 저장하지 않습니다.
    """
    global TEAM_MAP_CACHE
    if CURRENT_SLUG != "veta":
        return riders

    split_counts = dict(next(((c.get("test_split_counts") or {}) for c in CENTER_CONFIGS if c.get("slug") == "veta"), {}))
    if not split_counts:
        return riders
    ...
    for r in riders:
        stable_keys = stable_team_keys(r.get("name", ""), r.get("phone", ""), r.get("userId", ""))
        mapped = None
        for key in stable_keys:
            candidate = normalize_team_for_area((TEAM_MAP_CACHE or {}).get(key), AREA_NAME)
            if candidate in TEAM_ORDER:
                mapped = candidate
                break
        if mapped:
    ...
    # 화면/원본 순서 변화에 흔들리지 않도록 가나다순 + stable ID로 고정 정렬합니다.
    unassigned.sort(key=lambda r: (
        norm(r.get("name", "")),
        canonical_rider_key(r),
    ...
    for r in unassigned:
        ...
        for team in TEAM_ORDER:
            if counts.get(team, 0) < int(split_counts.get(team, 0)):
                target = team
                break
        if target is None:
            target = TEAM_ORDER[-1] if TEAM_ORDER else "팀3"

        r["team"] = target
        counts[target] = counts.get(target, 0) + 1
        for key in stable_team_keys(r.get("name", ""), r.get("phone", ""), r.get("userId", "")):
            updates[key] = target

    if updates:
        try:
            init_firebase()
            db.reference(TEAM_MAP_PATH).update(updates)
            if TEAM_MAP_CACHE is None:
                TEAM_MAP_CACHE = {}
            TEAM_MAP_CACHE.update(updates)
            print(f"VETA 테스트 팀 자동배정 저장: {len(unassigned)}명 / " + ", ".join(f"{t} {counts.get(t,0)}명" for t in TEAM_ORDER))
        except Exception as e:
            print("VETA 테스트 팀 자동배정 Firebase 저장 실패:", e)
    else:
        print("VETA 팀배정 유지: " + ", ".join(f"{t} {counts.get(t,0)}명" for t in TEAM_ORDER))
...
def finalize_rider_identity_and_teams(riders):
    ...
    riders = apply_veta_test_split(riders)
    return riders
...
def change_center(page, config):
    ...
    정식 DP코드가 있으면 기존처럼 DP코드로 엄격 검증합니다.
    VETA 테스트처럼 DP코드를 아직 모르는 경우 center_search(경산B)로 옵션을 찾아
    화면에서 DP코드를 자동 추출한 뒤 그 코드로 검증합니다.
    """
    global VERIFIED_CENTER_CODE
    VERIFIED_CENTER_CODE = None

    target_code = norm(config.get("center_code", ""))
    target_search = norm(config.get("center_search", ""))
    has_code = bool(re.fullmatch(r"DP\d+", target_code))
    if not has_code and not target_search:
        raise RuntimeError(f"{config['area']} center_code/center_search 설정이 없습니다.")

    print(f"협력사 변경 시도: {config['area']} / {target_code or target_search}")
    ...
    # 테스트 검색모드에서는 현재 코드가 무엇이든 목록에서 경산B 옵션을 직접 확인합니다.
    need_select = (not has_code) or current_code != target_code
    if need_select:
        ...
        selector = target_code if has_code else target_search
        ...
        (selector) => {
          ...
          const compact=s=>String(s||'').replace(/\s+/g,'');
          const sel=compact(selector);
          const matches=Array.from(document.querySelectorAll('body *'))
            .filter(visible)
            .filter(el=>{
              const txt=compact(el.textContent);
              return txt.includes(sel) && /DP\d+/.test(txt) && txt.length<120;
            })
            .sort((a,b)=>{
          ...
          if(!matches.length) return {ok:false,text:'',code:''};
          const raw=compact(matches[0].textContent);
          const m=raw.match(/DP\d+/);
          let el=matches[0];
          for(let i=0;i<6&&el;i++,el=el.parentElement){
            const r=el.getBoundingClientRect();
            const role=el.getAttribute&&el.getAttribute('role');
            const tag=(el.tagName||'').toLowerCase();
            if(r.height<160&&(role==='option'||tag==='li'||tag==='button')){
              el.click(); return {ok:true,text:raw,code:m?m[0]:''};
            }
          }
          matches[0].click();
          return {ok:true,text:raw,code:m?m[0]:''};
        }
        """, selector)
        if not selected or not selected.get("ok"):
            # 경산B라는 표시명이 화면에 없는 계정도 있습니다.
            # 이 경우 협력사 선택창에 보이는 DP코드가 정확히 1개일 때만 안전하게 자동 채택합니다.
            ...
            () => {
              ...
              for (const el of Array.from(document.querySelectorAll('body *')).filter(visible)) {
                const txt=compact(el.textContent);
                const m=txt.match(/DP\d+/);
                if(!m || txt.length>=120) continue;
                const code=m[0];
                if(!seen.has(code) || txt.length < seen.get(code).text.length) seen.set(code,{el,text:txt,code});
              }
              const vals=Array.from(seen.values());
              if(vals.length!==1) return {ok:false,codes:vals.map(v=>v.code),texts:vals.map(v=>v.text)};
              let el=vals[0].el;
              for(let i=0;i<6&&el;i++,el=el.parentElement){
                const r=el.getBoundingClientRect();
                const role=el.getAttribute&&el.getAttribute('role');
                const tag=(el.tagName||'').toLowerCase();
                if(r.height<160&&(role==='option'||tag==='li'||tag==='button')){
                  el.click(); return {ok:true,text:vals[0].text,code:vals[0].code,codes:[vals[0].code]};
                }
              }
              vals[0].el.click();
              return {ok:true,text:vals[0].text,code:vals[0].code,codes:[vals[0].code]};
            }
            """)
            if not fallback or not fallback.get("ok"):
                codes = ", ".join((fallback or {}).get("codes", [])) or "없음"
                raise RuntimeError(
                    f"{config['area']}({selector}) 표시명을 찾지 못했고, 협력사 DP코드도 1개로 확정할 수 없습니다. 후보: {codes}"
                )
            selected = fallback
            print(f"경산B 표시명 대신 단일 협력사 DP코드 자동 선택: {selected.get('code','')}")

        discovered_code = norm(selected.get("code", ""))
        if not has_code:
            if not re.fullmatch(r"DP\d+", discovered_code):
                raise RuntimeError(f"경산B 옵션에서 DP코드를 추출하지 못했습니다: {selected.get('text','')}")
            target_code = discovered_code
            config["center_code"] = target_code
            has_code = True
            print(f"VETA 경산B DP코드 자동 확인: {target_code}")
    ...
    if verified_code != target_code:
        raise RuntimeError(
            f"협력사 전환 검증 실패: 목표={target_code}, 실제={verified_code or '확인 실패'}; Firebase 업로드를 차단합니다."
        )

    VERIFIED_CENTER_CODE = verified_code
    config["center_code"] = verified_code
...
def main():
    ...
    with sync_playwright() as p:
        browser = p.chromium.launch_persistent_context(
            user_data_dir=str(BASE_DIR / "chrome_profile_veta"),
            headless=False,
        ...
        try:
            while True:
                ...
                print(f"{REFRESH_SECONDS}초 후 다시 경산B를 수집합니다.")
